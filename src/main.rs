...
mod pawn_moves {
    ...
    fn pawn_move_nn() {
        ...
        let moves = board.possible_wp();
        let correct_moves: Vec<Move> = vec![Move{from: 8, to: 16}, Move{from: 10, to: 18}, Move{from: 10, to: 26}, Move{from: 20, to: 28}, Move{from: 22, to: 30}];
        assert_eq!(moves.len(), correct_moves.len());
        for m in moves {
            assert!(correct_moves.contains(&m));
        }
    ...
    fn pawn_move_ss() {
        ...
        let moves = board.possible_bp();
        let correct_moves: Vec<Move> = vec![Move{from: 48, to: 40}, Move{from: 50, to: 42}, Move{from: 50, to: 34}, Move{from: 44, to: 36}, Move{from: 46, to: 38}];
        assert_eq!(moves.len(), correct_moves.len());
        for m in moves {
            assert!(correct_moves.contains(&m));
        }
